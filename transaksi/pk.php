...
if($op=='ambilbarang'){
    $data=mysql_query("select * from tblkamar");
    echo"<option>Kode Kamar</option>";
    while($r=mysql_fetch_array($data)){
        echo "<option value='$r[kode]'>$r[kode]</option>";
    }
}elseif($op=='ambildata'){
    $kode=$_GET['kode'];
    $dt=mysql_query("select * from tblkamar where kode='$kode'");
    ...
    $tot=$to['total'];
    $simpan=mysql_query("insert into transaksi(nonota,tanggal,total)
                        values ('$nota','$tanggal','$tot')");
    if($simpan){
        $query=mysql_query("select * from tblsementara");
        while($r=mysql_fetch_row($query)){
            mysql_query("insert into detailtransaksi(nonota,kode,harga,jumlah,subtotal)
                        values('$nota','$r[0]','$r[2]','$r[3]','$r[4]')");
            mysql_query("update tblkamar set stok=stok-'$r[3]'
                        where kode='$r[0]'");
        }
