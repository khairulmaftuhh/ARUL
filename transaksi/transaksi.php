            ...
            <div class="container">
                ...
                switch($p){
                    ...
                    case "tambah":
                        ...
                        $auto=mysql_query("select * from transaksi order by nonota desc limit 1");
                        ...
                        echo "<div class='navbar-form pull-right'>
                            ...
                            echo'<legend>Transaksi Pemesan</legend>
                             <form action="simpanreservasi.php" method="post">
							 <label>Nama pemesan</label>
							 
                            <input type="text" placeholder="Nama pemesan" name="namacus"><br>
                             <label>Jenis Kamar</label>
                            <select id="kode"></select>
                            <input type="text" id="harga" placeholder="Harga" class="span2" readonly><input type="text" id="stok" placeholder="stok" class="span1" readonly><br>
                             <label>Nama Kamar</label>
                            <input type="text" id="nama" placeholder="Nama kamar" readonly><br>
                            <label>tanggal masuk</label>
                            <input type="date" id="nama" placeholder="tanggal masuk" ><br>
                            <label>tanggal keluar</label>
                            <input type="date" id="nama" placeholder="tanggal keluar" ><br>
                            <label>Jumlah Tamu</label>
                            <input type="text" id="jumlah" placeholder="Jumlah Beli" class="span1"><br>
 
                            <button id="tambah" class="btn">Tambah</button><br><br>
                            <span id="status"></span>
                            <div class="form-actions">
                            <a href="index.php?page=penjualan">    <button type="submit" name="submit" id="proses">Proses</button> </a>
                            </div>
							<form>';
                        break;
                    case "detail":
                        echo "<legend>Nota Transaksi</legend>";
                        $nota=$_GET['nota'];
                        $query=mysql_query("select transaksi.nonota,detailtransaksi.kode,tblkamar.nama,
                                           detailtransaksi.harga,detailtransaksi.jumlah,detailtransaksi.subtotal
                                           from detailtransaksi,transaksi,tblkamar
                                           where transaksi.nonota=detailtransaksi.nonota and tblkamar.kode=detailtransaksi.kode
                                           and detailtransaksi.nonota='$nota'");
                        $nomor=mysql_fetch_array(mysql_query("select * from transaksi where nonota='$nota'"));
                        echo "<div class='navbar-form pull-right'>
